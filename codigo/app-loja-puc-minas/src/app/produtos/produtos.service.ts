...
import {API_FORNECEDOR} from "../app.api";
...
export class ProdutosService {
    ...
    getTodosProdutos():  Observable<Produto []> {
        return this.http.get(`${API_FORNECEDOR}/produtos`)
    ...
    avaliacoesDosprodutos(id: string): Observable<Produto>{
        return this.http.get(`${API_FORNECEDOR}/produtos/${id}/avaliacoes`)
        .map(response => response.json())
        .catch(ErrorHandler.handlerError)
    }

    getProdutoPorId(id: Number):  Observable<Produto> {
        return this.http.get(`${API_FORNECEDOR}/produtos/${id}`)
        .map(response => response.json())
