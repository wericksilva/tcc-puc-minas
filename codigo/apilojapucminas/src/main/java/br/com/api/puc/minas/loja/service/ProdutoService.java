...
public class ProdutoService {
	...
	public List<Produto> buscarTodos() {
		...
		return produtoRepository.findAll().stream().filter((Produto p) -> p.getAtivo() == true)
				.collect(Collectors.toList());
	}

	public List<Produto> buscarTodosQuery(String query) {

		return produtoRepository.findByNomeContaining(query).stream().filter((Produto p) -> p.getAtivo() == true)
				.collect(Collectors.toList());
	}
	...
	public Produto salvar(Produto produto) {
		Produto pacienteIncluido = produtoRepository.save(produto);
